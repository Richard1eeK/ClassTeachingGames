...
**最新 tag**：`v3.5.2` — Manually Type-in 单词列表字体加粗
...
- **代码版本**：`main` 位于 v3.5.2，素材库和课堂交互打磨基本完成
- **素材库**：Bright Spark 284 张闪卡（33/37 类有内容）+ High Flyer 1,233 张图片 + 42 个词库文件
- **macOS 验证**：全天所有改动通过 py_compile + smoke test
- **Windows 验证**：用户下次 `git pull` + `build.bat` 需重点确认 Manually Type-in 单词列表文字在一体机上更粗更清晰，右侧常驻触摸条仍可见且可拖动
- **版本号**：v3.5.2 显示于主界面右下角
...
**主线**：Windows 端 build.bat 实测 v3.5.2，确认 Manually Type-in 单词列表行文字更粗更易读，右侧常驻触摸条不导入也可见，导入大量词汇后可拖动滚动。
...
```bash
git reset --hard v3.5.2        # Manually Type-in 单词列表字体加粗
