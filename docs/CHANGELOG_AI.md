...
从 v1.0-stardew (2026-05-20) 到 v3.5.2 (2026-05-31)，共 29 个版本，历时 12 天。

| 版本 | 日期 | 主题 | 类型 |
|------|------|------|------|
| **v3.5.2** | 2026-05-31 | 单词列表字体加粗 | 🔧 修复 |
| **v3.5.1** | 2026-05-31 | 常驻触摸条可见性修复 | 🔧 修复 |
...
- **v3.0-v3.5.2**：内置材料库（High Flyer + Bright Spark 37 类） + 课堂可用性打磨
...
**问题**：Manually Type-in 单词列表行文字在一体机上显得偏细，尤其图片文件名列表远距离看比较费眼。

**修复内容**：
- `game/item_list.py` — 列表行文字从 `FONT_CAPTION` 常规字重改为 `FONT_BODY` 加粗，只影响 Manually Type-in 的 item rows。
- `game/settings.py` — 版本号更新为 v3.5.2。

**验证**：
- ✅ `python3 -m py_compile game/item_list.py game/settings.py`
- ✅ 静态检查确认列表行文字渲染为 `T.FONT_BODY` + `bold=True`
- ⏳ Windows 端 build.bat + 一体机实测待用户确认

### v3.5.1 — 常驻触摸条可见性修复

**问题**：v3.5 的滚动条只有词汇数量超过列表高度时才绘制，而且 12px 木色轨道太窄、太像边框；不符合“右侧触摸条常驻且清楚可见”的课堂触摸屏需求。

**修复内容**：
- `game/item_list.py` — 列表右侧始终预留 34px 触摸槽，不滚动时也显示满高金色把手。
- `game/item_list.py` — 轨道加深、把手加宽到 16px，并加入三条 grip 线，电脑和触摸屏上都更容易看到。
- `game/settings.py` — 版本号更新为 v3.5.1。

**验证**：
- ✅ `python3 -m py_compile game/item_list.py game/settings.py`
- ✅ dummy SDL geometry smoke：短列表也存在 34px 触摸槽和满高把手；长列表把手缩短，拖动后 `scroll_offset > 0`
