...
### D-015: v1.9 多答案采用 A1 设置入口 + Normal 规则

**背景**：用户希望新增难度玩法：杯子中有 2 或 3 个正确答案，杯子数量强制为答案数量 + 2。需要让设置入口一眼看懂。

**候选**：
- A1. **调 Correct Answers，自动显示 Cups = Answers + 2** ← 选定
- A2. 调 Cups，自动显示 Answers = Cups - 2
- B. Answer slider + 自动杯数说明
- C. 单独 Advanced 设置页

**选定 A1 的理由**：
- 新玩法的核心是“要找几个正确答案”，不是单纯调杯子数量
- `1 Ans / 3 Cups`、`2 Ans / 4 Cups`、`3 Ans / 5 Cups` 三张卡比滑块更直观
- 不把核心玩法藏进高级页，课堂上老师能快速理解并切换
