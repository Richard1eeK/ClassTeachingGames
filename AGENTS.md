...
- **AGENTS.md（本文件）**：长期有效的运行方式、技术栈、目录结构、代码风格、禁止事项、用户协作偏好
...
- 默认不写注释，除非 *Why* 不显然（参考根 `~/AGENTS.md` 全局规则）
...
- **输出风格**：用户偏好简洁、直接、有结构的回复。每次回复开头加 emoji，结尾加 `Your Honour`（这是项目级 AGENTS.md 已有的规则）。
